<template>
  <ProLayout>
    ...
    <ProLayoutMain>
      <ProLayoutSection title="用户列表" prefix="line" fill>
        <template #title-extra>
          <a-button type="primary" @click="onAddUser" v-permission="'user:search'">
        ...
        >
          <template #bodyCell="{ column, text, record }">
            ...
            <template v-if="column.dataIndex === 'operator'">
              <a-button type="link" size="small" @click="onEdit(record)" v-permission="'user:edit'"
                >编辑</a-button
              >
              ...
              >
                <a-button type="link" size="small" v-permission="'user:delete'">删除</a-button>
              </a-popconfirm>
            </template>
          </template>
        </a-table>
...
const getRoleList = async () => {
  ...
  roleList.value = (response?.data?.result?.records || []).map((item) => {
    return {
      label: item.roleName,
      value: item.id,
    }
