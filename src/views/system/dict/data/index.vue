<template>
  <ProLayout>
    ...
    <ProLayoutMain>
      <ProLayoutSection title="字典数据列表" prefix="line" fill>
        <template #title-extra>
          <a-button type="primary" @click="onAddUser" v-permission="'dict:add'">
        ...
        >
          <template #bodyCell="{ column, text, record }">
            ...
            <template v-if="column.dataIndex === 'dictCode'">
              <a-button type="link" size="small" @click="onJumpDetail(record)">{{
                record.dictCode
              }}</a-button>
            </template>
            <template v-if="column.dataIndex === 'operator'">
              <a-button type="link" size="small" @click="onEdit(record)" v-permission="'dict:edit'"
                >编辑</a-button
              >
              ...
              >
                <a-button type="link" size="small" v-permission="'dict:delete'">删除</a-button>
              </a-popconfirm>
            </template>
          </template>
        </a-table>
...
import DictTypeOperatorModal from './operator.vue'

import { getDictDataListApi, deleteDictDataApi } from '@/api/system/dict'

import dayjs from 'dayjs'
import { columns, STATUS_OPTIONS, SEARCH_ITEMS, MODAL_TYPE } from './options'
...
const dictCode = computed(() => route.params.dictCode)
...
const getData = async () => {
  loading.value = true
  const response = await getDictDataListApi(dictCode.value, {
...
const onDelete = async (record) => {
  await deleteDictDataApi(dictCode.value, record.id)
